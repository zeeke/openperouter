...
grout-deploy-operator-with-olm: bundle kustomize kind clab-cluster load-on-kind deploy-olm
	sed -i 's|quay.io/openperouter/router:main$|quay.io/openperouter/router:main-grout|g' $(CSV_FILE)
	$(MAKE) build-and-push-bundle-images IMG_TAG=main-grout
	$(MAKE) deploy-operator-with-olm IMG_TAG=main-grout

.PHONY: grout-deploy-helm
grout-deploy-helm: IMG_TAG=main-grout
grout-deploy-helm: HELM_ARGS=--set openperouter.datapaht=grout
