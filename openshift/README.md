...
# Prefetch generic dependencies
podman run --rm -ti -v "$PWD:$PWD:z" -w "$PWD" ghcr.io/hermetoproject/hermeto:latest fetch-deps --source ./openshift --output ./bin/cachi2/output/ generic

# Build the image
podman build -v "$TMPDIR/entitlement:/run/secrets/etc-pki-entitlement:Z"  \
               -v "$TMPDIR/rhsm:/run/secrets/rhsm:Z" \
               -v "$PWD/bin/cachi2:/cachi2" \
