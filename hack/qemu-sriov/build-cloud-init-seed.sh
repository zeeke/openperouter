...
ssh-keygen -t ed25519 -N "" -f "$QEMU_WORKDIR/id_ed25519"

cat > "$QEMU_WORKDIR/meta-data" <<'EOF'
...
cat > "$QEMU_WORKDIR/user-data" <<EOF
#cloud-config
ssh_authorized_keys:
  - $(cat "$QEMU_WORKDIR/id_ed25519.pub")
package_update: true
write_files:
...
cloud-localds "$QEMU_WORKDIR/seed.img" "$QEMU_WORKDIR/user-data" "$QEMU_WORKDIR/meta-data"
