...
func configureGroutDataPath(ctx context.Context, config groutConfiguration) error {
	...
	if hasAlreadyUnderlay && len(config.Underlays) == 0 {
		...
		if err := grout.RemoveAllVNIs(config.targetNamespace); err != nil {
	...
	for _, vni := range hostConfig.L2VNIs {
		...
		if err := grout.SetupL2VNI(ctx, vni); err != nil {
			return fmt.Errorf("failed to setup vni: %w", err)
		}
	...
	toCheck := make([]grout.VNIParams, 0, len(hostConfig.L3VNIs)+len(hostConfig.L2VNIs))
	for _, vni := range hostConfig.L3VNIs {
	...
	if err := grout.RemoveNonConfiguredVNIs(config.targetNamespace, toCheck); err != nil {
