...
func (g *GroutDatapathConfigurator) Configure(ctx context.Context, config interfacesConfiguration) error {
	...
	if len(currentUnderlayIfaces) > 0 && len(config.Underlays) == 0 {
		restoreUnderlayGrout(ctx, groutClient, config.targetNamespace, currentUnderlayIfaces)
		return nil
	}
...
func restoreUnderlayGrout(ctx context.Context, groutClient *grout.Client, targetNamespace string, currentUnderlayIfaces []hostnetwork.UnderlayInterface) {
	slog.InfoContext(ctx, "underlay removed, cleaning up VNIs, L3VPNs and underlay interfaces")
