...
func SetupUnderlay(ctx context.Context, client *Client, params hostnetwork.UnderlayParams) error {
	...
	// Read the underlay IPs (assigned by infra, e.g. containerlab) and strip
	// them from the kernel interface so only grout owns them.
	...
	devargs := fmt.Sprintf("net_tap0,remote=%s,iface=%s", params.UnderlayInterface, underlayTapName)
	...
	// Grout creates a NOARP kernel interface named after the port with /32
	// addresses. Add the full subnet addresses so the kernel has a connected
	// route for the underlay subnet — needed by FRR bgpd's kernel TCP.
	// NOARP prevents the kernel from responding to ARP (grout handles ARP).
	if err := netnamespace.In(ns, func() error {
		link, err := netlink.LinkByName(underlayPortName)
		if err != nil {
			return fmt.Errorf("grout kernel interface %s not found: %w", underlayPortName, err)
		}
		for _, addr := range underlayAddrs {
			a := addr
			a.Label = ""
			if err := netlink.AddrReplace(link, &a); err != nil {
				slog.WarnContext(ctx, "failed to add subnet address to grout interface",
					"addr", addr.IPNet, "iface", underlayPortName, "error", err)
			}
		}
		return nil
	}); err != nil {
		return fmt.Errorf("failed to configure grout kernel interface: %w", err)
	}
