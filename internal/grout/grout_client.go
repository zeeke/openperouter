...
func (d *groutInterfaceProperties) matchesRequested(devargs string, opts PortOptions) bool {
	...
	if opts.Promiscuous != nil && hasPromiscFlag(d.Flags) != *opts.Promiscuous {
		return false
	}
	return true
}
...
func hasPromiscFlag(flags []string) bool {
	// when promisc is set, the flags field contains "promisc". if it's not set, the flags field
	// does not contain "promisc", even if the port has been created with explicit`promisc off`
	//
	// grout# interface add port p1 devargs net_null1 promisc on
	// grout# interface show name p1
	// ...
	// flags: up running promisc allmulti
	//
	// grout# interface add port p2 devargs net_null2 promisc off
	// grout# interface show name p2
	// flags: up running allmulti
	return slices.Contains(flags, "promisc")
}
