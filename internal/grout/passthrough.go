...
func SetupPassthrough(ctx context.Context, client *Client, params hostnetwork.PassthroughParams) error {
	...
	if err := ensurePortAddresses(ctx, client, portName, params.HostVeth.NSIPv4, params.HostVeth.NSIPv6); err != nil {
...
func ensureTapPortInHostNamespace(ctx context.Context, client *Client, portName, tapName, vrf string, groutNs netns.NsHandle) error {
	...
	if !portExists {
		...
		devargs := fmt.Sprintf("net_tap%d,iface=%s", makeTapIndex(), tapName)
...
func ensurePortAddresses(ctx context.Context, client *Client, portName string, ipv4, ipv6 string) error {
	...
	if err := client.ensureAddress(ctx, portName, ipv4); err != nil {
		return fmt.Errorf("failed to assign IP %s to port %s: %w", ipv4, portName, err)
	}
	if err := client.ensureAddress(ctx, portName, ipv6); err != nil {
		return fmt.Errorf("failed to assign IP %s to port %s: %w", ipv6, portName, err)
	}
