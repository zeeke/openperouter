...
func SetupPassthrough(ctx context.Context, client *Client, params hostnetwork.PassthroughParams) error {
	...
	if !portExists {
		if err := createPassthroughPort(ctx, client, portName, tapName, ns); err != nil {
			return err
		}
	} else {
...
func createPassthroughPort(ctx context.Context, client *Client, portName, tapName string, ns netns.NsHandle) error {
