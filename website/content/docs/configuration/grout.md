...
- `--grout-enabled` and `--grout-socket` flags to the controller
...
|-------|------|---------|-------------|
| `grout.enabled` | boolean | `false` | Enable the grout sidecar for DPDK-accelerated forwarding |
| `grout.image.repository` | string | `quay.io/grout/grout` | Grout container image repository |
| `grout.image.tag` | string | `0.15.0` | Grout container image tag |
