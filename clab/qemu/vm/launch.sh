...
NIC_ARGS=""
UDEV_RULES="# Auto-generated by launch.sh — maps igb MACs to bridge names."
slot=1
...
for nic in "${NICS[@]}"; do
    ...
    UDEV_RULES+=$'\n'"SUBSYSTEM==\"net\", ACTION==\"add\", ATTR{address}==\"${mac}\", NAME=\"${nic}\""
...
echo "Setting up udev rules for NIC naming..."
ssh ${SSH_OPTS} -p "${SSH_PORT}" openperouter@localhost \
    "sudo tee /etc/udev/rules.d/70-persistent-net.rules" <<< "${UDEV_RULES}"

#ssh ${SSH_OPTS} -p "${SSH_PORT}" openperouter@localhost \
#    "cat /etc/udev/rules.d/70-persistent-net.rules"

# Reboot to apply udev rules and IOMMU cmdline
echo "Rebooting VM to apply udev rules and IOMMU cmdline ..."
ssh ${SSH_OPTS} -p "${SSH_PORT}" openperouter@localhost \
    "sudo reboot" || true
