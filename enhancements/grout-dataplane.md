...
2. controller creates a grout port for the underlay, linking it to the eth3 nic:
   ...
   `grcli interface add port underlay dev_args net_tap0,remote=eth3,iface=tap_underlay`

3. move all the ip addresses from eth3 to the grout underlay port
  
   `grcli interface add <eth3_ip> iface underlay

4. set kernel route for all the moved ip addresses to use grout. `main` is the default vrf 
   control plane interface. This is needed to make sure all the connection creted by FRR
   go through Grout
...
L3Passthrough
1. The controller create a pair grout_port/tap_interface (compared to the veth pair of kernel datapath):

   `grcli interface add port pt-ns devargs net_tap1,iface=pt-host`

   `address add address add 192.169.10.1 iface pt-ns192.169.10.1 iface pt-ns`

2. The controller move the pt-host tap interface to the host namespace and configures it

   `ip link set pt-host netns <host>`
   
   `ip addr add 192.169.10.2 dev pt-host`
...
#### M1a L3Passthrough HW acceleration
_depends on M1_
...
- When the Underlay nic is an SR-IOV capable NIC configured with VFs, then the PERouter uses one VF
  to communicate with the fabric. Other 2 VFs will be used as Veth pair by tuning them on the same
  VLAN. One leg of this VF pair will be attached to grout and configured with a DPDK driver. The other
  leg will be moved to the host namespace, binding it to a kernel driver.
